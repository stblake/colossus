// Compressocrat cipher generator (test-data tool, not part of the solver).
//
...
// ascending -- exactly what the solver searches), and enciphers with compressocrat_encrypt. It
// links the REAL cipher code (compressocrat.c + utils.c) so the generator and the solver can
// never drift in convention.
//
//   make compressocrat_gen
//   ./compressocrat_gen plaintext.txt KEYWORD >cipher.txt 2>solution.txt
//
// argv: <plaintext-file|-> <keyword>
// stdout: the Compressocrat ciphertext (one line, bare A..Z; C differs from N -- usually shorter)
...
int main(int argc, char **argv) {
    ...
    int clen = compressocrat_encrypt(raw, n, sigma, cipher);
