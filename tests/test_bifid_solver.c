//
//  In-process stress / limits tests for the Bifid solver (solve_cipher).
//
...
//  keyed square and period, then attack it. The suite does four things:
...
//    2. tests the PERIOD ESTIMATOR in isolation (bifid_estimate_periods) -- the true
...
//       pinned to isolate square recovery) so the short-text cliff is visible.
//
//  Run from the source directory so the n-gram table is found in the cwd. Bifid, like
//  Playfair, needs the discriminating log-probability scoring, so this enables
...
#include "bifid_solver.h"  // bifid_estimate_periods
...
// Plant a Bifid cipher: take the first pt_len letters of PLAINTEXT, build the square
// from `keyword`, encipher with `period`. Fills prepared[] (the expected solution, J->I
// merged) and cipher_str (the A..Z ciphertext). Returns the length.
static int plant(const char *keyword, int pt_len, int period, int prepared[], char cipher_str[]) {
    ...
    bifid_encrypt(prepared, n, grid, 5, period, cipher);
...
static void test_registry(void) {
    ...
    init_config(&cfg);
    cfg.cipher_type = BIFID; cfg.method = METHOD_DEFAULT;
    CHECK(apply_cipher_defaults(&cfg, false), "bifid registry: no entry applied");
    CHECK(cfg.n_restarts == 4 && cfg.n_hill_climbs == 200000,
        "bifid anneal defaults wrong: %dx%d", cfg.n_restarts, cfg.n_hill_climbs);
    CHECK(cfg.init_temp > 0.0799 && cfg.init_temp < 0.0801,
        "bifid anneal inittemp wrong: %.4f", cfg.init_temp);
...
static void test_period_estimator(void) {
    ...
    for (int pi = 0; pi < 5; pi++) {
        for (int li = 0; li < 2; li++) {
            ...
            for (int i = 0; i < plen; i++) cidx[i] = letter_to_index((unsigned char) cipher_str[i]);

            int out[8];
            int n = bifid_estimate_periods(cidx, plen, 2, 20, 5, out, false);
            trials++;
            int found = 0;
...
static void test_capability(void) {
    ...
    // Estimate the period (period == 0) and break the square at the registry default.
    int got_period = -1;
    double secs = 0.0;
    double frac = solve_and_frac(cipher_str, prepared, plen, true,
        0, 0, 0.0, 0.0, 0, 0, 1u, &got_period, &secs);
    printf("[capability registry-anneal 760ch, period estimated] frac=%.3f period=%d %.1fs\n",
        frac, got_period, secs);
    CHECK(frac >= 0.95, "bifid 760ch (period estimated) recovered only %.3f", frac);
    CHECK(got_period == period, "bifid recovered period %d, planted %d", got_period, period);
}

// --- 4. length cliff (period pinned to isolate square recovery) ---------------

static void test_length_cliff(void) {
    int lengths[] = {200, 350, 500, 760};
    printf("recovery vs ciphertext length (keyword KRYPTOSABCDEF, period 7 pinned, 4x200k anneal, seed 1):\n");
    ...
    for (int li = 0; li < 4; li++) {
        ...
        double frac = solve_and_frac(cipher_str, prepared, plen, false,
            4, 200000, 0.08, 0.30, 7, 1, 1u, NULL, &secs);
    ...
    CHECK(frac_longest >= 0.95, "bifid 760ch (cliff sweep) recovered only %.3f", frac_longest);
}

int main(void) {
    init_alphabet("J");                 // 25-letter Bifid alphabet (J merged into I)
    CHECK(g_alpha == PLAYFAIR_GRID, "alphabet size %d, expected %d", g_alpha, PLAYFAIR_GRID);

    g_ngram_logprob = true;             // Bifid needs the log-probability fitness
